...
- `code/seal/capture/`: The capture scaffold. A decorator that binds a function's arguments, runs it, then appends a commitment to those arguments and the run itself to the assignment's chain, optionally asks an external witness to cosign, and checks the result against the real verifier before writing a manifest. Opt-in per function, which is why it makes a run count sayable without making it enforced.
...
**A hosted decision model showed the same two gaps outside valuation, observed incidentally.** On 2026-09-25 a hosted classification model, reached through a third-party gateway, was asked one fixed question about fixed text ten times, with byte-identical requests. Its probabilities moved by up to 0.09 between runs, enough to change which side of a fixed threshold the answer fell on. The gateway returned the requested model name on every call and never a version. A later check traced this to a documented, unresolved defect in the gateway rather than the model: the model does track a resolved version, and the gateway drops it before the response reaches the caller. The operator's own log recorded each call, and nothing in it showed whether a kept run was the one run or one of ten. This was not a pre-registered check, it involved no valuation, and the operator held every input, so it does not bear on the fourth condition. What it shows, at small scale, is the pattern above: re-running does not recover what an earlier run returned, and what an operator's log leaves open is the count.
