...
**The `/instrument` row is now historical, confirmed against the live page on 2026-09-17.** The live page is built on the canonical five. Completeness and Coverage sit on the record axis. Disclosure, Anchoring, and the timestamp-service determination sit under supporting determinations. Same treatment as the Demo output row: the row above still describes what the page listed before the rewrite and is left as the historical diagnosis, not the current state.
